...
class snp_6hr_2byte(object):
    def __init__(self, vtype=None, dbbaseDir=None):
        ...
        self.vars, \
        self.coefs   = list(zip(*list(dictvars.items())))
        self.dictvars= dictvars

    def load_6hr(self, vname,scen,ens,DTime, miss_fill=None):
        """
        Precipitation: Forward 6-hour average (confirmed by comparison with daily precipitaiton data)
        """
        ...
        nsteps     = int(os.stat( srcPath ).st_size / (ny*nx*nvar*2)) # num of timesteps
...
def LatBnd():
    a1lat = Lat()
    a1latbnd = [-90] + list((a1lat[:-1]+a1lat[1:])*0.5) + [90]
    return np.array(a1latbnd)

def LonBnd():
    a1lon = Lon()
    dlon  = dLon()
    a1lonbnd = [a1lon[0]-dlon*0.5]+ list((a1lon[:-1]+a1lon[1:])*0.5) + [a1lon[-1]+dlon*0.5]
    return np.array(a1lonbnd)
